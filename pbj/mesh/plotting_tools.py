...
def plot_surface(
    ...
    if not min_max_vals:
        val_max = max_colorbar_scale * max(
            abs(np.max(surface_values)), abs(np.min(surface_values))
        )
        val_min = -val_max
    ...
    color_codes = ["rgb({0}, {1}, {2})".format(*colorfun(x)) for x in surface_values]
    color_codes = np.array(color_codes)

    if stern_mesh:
        solute_mesh = stern_mesh.mesh
    else:
    ...
    potential_surf.add_trace(colorbar_trace)
    fig = potential_surf
...
def plot_multiple_surfaces(
    ...
    if not min_max_vals:
        ...
        for solute in simulation.solutes:
            phi_vertices, _ = solute.get_surface_potential(
