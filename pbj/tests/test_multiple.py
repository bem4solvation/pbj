...
def test_multiple():
    ...
    def spheres1():
        ...
        for mesh_dens in [2, 4, 8, 16]:
            sphere = pbj.implicit_solvent.Solute(pqrpath, mesh_density=mesh_dens, mesh_generator="msms")
    ...
    for j in range(len(spheres1)):
        print(f'Calculating solvation energy/forces for mesh density {spheres1[j].mesh_density}')
        ...
        simulation_mult.calculate_solvation_forces(force_formulation='energy_functional')
        f_solv_energy_func = spheres1[j].results['f_solv']
        simulation_mult.calculate_solvation_energy()
        energy_val_multiple = spheres1[j].results["electrostatic_solvation_energy"] + \
            spheres2[j].results["electrostatic_solvation_energy"]
        
        energy_val = energy_val_multiple - 2*energy_val_single
        file.write(f"Mesh density: {spheres1[j].mesh_density}, Binding energy energy: {energy_val:.4f} " \
